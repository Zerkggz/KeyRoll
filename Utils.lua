...
local function StoreGuildKey(sender, mapID, level, class)
    ...
    -- If this person is in our current party, also store in party cache
    if IsInRealParty() then
        local partyMembers = GetCurrentPartyMembers()
        if partyMembers[shortName] then
            cache[shortName] = {
                name = shortName,
                fullName = fullName,
                realm = realm,
            ...
            if IsDebug() then
                DebugPrint("Also stored guild key in party cache (guild member in current party):", shortName)
            end
        end
    end
