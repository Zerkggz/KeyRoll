...
C_ChatInfo.RegisterAddonMessagePrefix("KeyRoll")
...
local function RequestPartyKeystones(force, silent)
    ...
    if KeyRoll.IsInRealParty() then
        ...
        -- Request from other KeyRoll users
        pcall(C_ChatInfo.SendAddonMessage, "KeyRoll", "REQUEST", chatType)
    ...
    if KeyRoll.IsDebug() then
        KeyRoll.DebugPrint("Sent keystone requests to party (KeyRoll + 7 addon prefixes)")
    end
end
...
chatFrame:SetScript("OnEvent", function(_, event, message, sender)
    ...
    -- message and sender can be taint-protected "secret" values during instances
    local msgOk = pcall(string.find, message, "")
    if not msgOk then return end
    
    local senderOk, ambiguatedName = pcall(Ambiguate, sender, "short")
    if not senderOk or not ambiguatedName then
        return
    end
...
bnetFrame:SetScript("OnEvent", function(_, event, prefix, message, distribution, sender)
    ...
    -- If friend is in our current party, also store in party cache
    if KeyRoll.IsInRealParty() then
        local partyMembers = KeyRoll.GetCurrentPartyMembers()
        if partyMembers and partyMembers[shortName] then
            KeyRoll.StoreKey(fullName, mapID, keyLevel)
            if KeyRoll.IsDebug() then
                KeyRoll.DebugPrint("Also stored in party cache (friend is in current party)")
            end
        end
    end
